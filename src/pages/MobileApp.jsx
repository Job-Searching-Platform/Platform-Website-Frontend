const MobileApp = () => {
  return (
    <section className="relative pt-20 pb-20 lg:pb-32">
      <img
        className="absolute top-0 left-0 mt-32 hidden lg:block"
        src=" https://shuffle.dev/zeus-assets/icons/dots/blue-dot-left-bars.svg"
        alt=""
      />
      <img
        className="absolute top-0 right-0 mt-32 hidden lg:block"
        src=" https://shuffle.dev/zeus-assets/icons/dots/yellow-dot-right.svg"
        alt=""
      />
      <div className="container relative mx-auto px-4">
        <div className="mx-auto mb-12 max-w-2xl text-center lg:mb-20">
          <span
            className="text-xs font-semibold text-blue-400"
            data-config-id="label"
          >
            What&apos;s new at Shuffle
          </span>
          <h2
            className="font-heading mt-8 mb-10 text-4xl font-semibold"
            data-config-id="header"
          >
            Get our Free Mobile App
          </h2>
          <p className="mb-10 text-xl text-gray-500" data-config-id="desc">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit.
            Pellentesque massa nibh, pulvinar vitae aliquet nec, accumsan
            aliquet orci.
          </p>
          <div className="mb-16 flex items-center justify-center">
            <a className="mr-4" href="/">
              <img
                src=" https://shuffle.dev/zeus-assets/buttons/app-store.svg"
                alt=""
              />
            </a>
            <a href="/">
              <img
                src=" https://shuffle.dev/zeus-assets/buttons/google-play.svg"
                alt=""
              />
            </a>
          </div>
        </div>
        <div className="-mx-4 -mb-8 flex flex-wrap items-center">
          <div className="w-full px-4 lg:w-2/6">
            <div className="mb-8 flex">
              <span className="mr-4 flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-teal-400">
                <svg
                  width="18"
                  height="21"
                  viewbox="0 0 18 21"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M8.98451 19.606C8.98451 19.606 16.6565 17.283 16.6565 10.879C16.6565 4.474 16.9345 3.974 16.3195 3.358C15.7035 2.742 9.99051 0.75 8.98451 0.75C7.97851 0.75 2.26551 2.742 1.65051 3.358C1.03451 3.974 1.31251 4.474 1.31251 10.879C1.31251 17.283 8.98451 19.606 8.98451 19.606Z"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M6.38593 9.87463L8.27793 11.7696L12.1759 7.86963"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                </svg>
              </span>
              <div>
                <h3
                  className="font-heading mb-4 text-2xl font-semibold"
                  data-config-id="header1"
                >
                  Security in saving
                </h3>
                <p className="text-lg text-gray-500" data-config-id="desc1">
                  Pellentesque eu quam vitae mi lacinia consequat quis in metus.
                </p>
              </div>
            </div>
            <div className="mb-8 flex">
              <span className="mr-4 flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-400">
                <svg
                  width="22"
                  height="20"
                  viewbox="0 0 22 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M16.9026 6.8512L12.4593 10.4642C11.6198 11.1302 10.4387 11.1302 9.59919 10.4642L5.11841 6.8512"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M15.9089 19C18.9502 19.0084 21 16.5095 21 13.4384V6.57001C21 3.49883 18.9502 1 15.9089 1H6.09114C3.04979 1 1 3.49883 1 6.57001V13.4384C1 16.5095 3.04979 19.0084 6.09114 19H15.9089Z"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                </svg>
              </span>
              <div>
                <h3
                  className="font-heading mb-4 text-2xl font-semibold"
                  data-config-id="header2"
                >
                  Weekly email updates
                </h3>
                <p className="text-lg text-gray-500" data-config-id="desc2">
                  Pellentesque eu quam vitae mi lacinia consequat quis in metus.
                </p>
              </div>
            </div>
            <div className="mb-8 flex">
              <span className="mr-4 flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-blue-400">
                <svg
                  width="20"
                  height="20"
                  viewbox="0 0 20 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M10.0001 0.750122C15.1081 0.750122 19.2501 4.89112 19.2501 10.0001C19.2501 15.1081 15.1081 19.2501 10.0001 19.2501C4.89112 19.2501 0.750122 15.1081 0.750122 10.0001C0.750122 4.89112 4.89112 0.750122 10.0001 0.750122Z"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M9.99524 6.20422V10.6232"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M9.995 13.7961H10.005"
                    stroke="white"
                    strokeWidth="2"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                </svg>
              </span>
              <div>
                <h3
                  className="font-heading mb-4 text-2xl font-semibold"
                  data-config-id="header3"
                >
                  No errors
                </h3>
                <p className="text-lg text-gray-500" data-config-id="desc3">
                  Pellentesque eu quam vitae mi lacinia consequat quis in metus.
                </p>
              </div>
            </div>
          </div>
          <div className="mt-16 mb-16 w-full px-4 lg:mt-0 lg:mb-0 lg:w-2/6">
            <img
              className="lg:max-h-128 mx-auto max-h-96 w-full lg:w-3/4"
              src=" https://shuffle.dev/zeus-assets/images/devices/phone-full.svg"
              alt=""
              data-config-id="07_image"
            />
          </div>
          <div className="w-full px-4 lg:w-2/6">
            <div className="mb-8 flex">
              <span className="mr-4 flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-red-400">
                <svg
                  width="18"
                  height="20"
                  viewbox="0 0 18 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M13.4235 7.4478V5.3008C13.4235 2.7878 11.3855 0.7498 8.87249 0.7498C6.35949 0.7388 4.31349 2.7668 4.30249 5.2808V5.3008V7.4478"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M12.6832 19.2496H5.04224C2.94824 19.2496 1.25024 17.5526 1.25024 15.4576V11.1686C1.25024 9.07359 2.94824 7.37659 5.04224 7.37659H12.6832C14.7772 7.37659 16.4752 9.07359 16.4752 11.1686V15.4576C16.4752 17.5526 14.7772 19.2496 12.6832 19.2496Z"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M8.86292 12.2028V14.4238"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                </svg>
              </span>
              <div>
                <h3
                  className="font-heading mb-4 text-2xl font-semibold"
                  data-config-id="header4"
                >
                  Change of access
                </h3>
                <p className="text-lg text-gray-500" data-config-id="desc4">
                  Pellentesque eu quam vitae mi lacinia consequat quis in metus.
                </p>
              </div>
            </div>
            <div className="mb-8 flex">
              <span className="mr-4 flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-yellow-400">
                <svg
                  width="24"
                  height="20"
                  viewbox="0 0 24 20"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M22.5 10.8055H1.5"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M20.6299 6.5951V5.0821C20.6299 3.0211 18.9589 1.3501 16.8969 1.3501H15.6919"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M3.37012 6.5951V5.0821C3.37012 3.0211 5.04112 1.3501 7.10312 1.3501H8.33912"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M20.6299 10.8046V14.8786C20.6299 16.9406 18.9589 18.6116 16.8969 18.6116H15.6919"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M3.37012 10.8046V14.8786C3.37012 16.9406 5.04112 18.6116 7.10312 18.6116H8.33912"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                </svg>
              </span>
              <div>
                <h3
                  className="font-heading mb-4 text-2xl font-semibold"
                  data-config-id="header5"
                >
                  Frequent authentication
                </h3>
                <p className="text-lg text-gray-500" data-config-id="desc5">
                  Pellentesque eu quam vitae mi lacinia consequat quis in metus.
                </p>
              </div>
            </div>
            <div className="mb-8 flex">
              <span className="mr-4 flex h-12 w-12 flex-shrink-0 items-center justify-center rounded-full bg-gray-900">
                <svg
                  width="22"
                  height="18"
                  viewbox="0 0 22 18"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M16.8877 7.89673C18.2827 7.70073 19.3567 6.50473 19.3597 5.05573C19.3597 3.62773 18.3187 2.44373 16.9537 2.21973"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M18.7285 11.2502C20.0795 11.4522 21.0225 11.9252 21.0225 12.9002C21.0225 13.5712 20.5785 14.0072 19.8605 14.2812"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M10.8867 11.6638C7.67273 11.6638 4.92773 12.1508 4.92773 14.0958C4.92773 16.0398 7.65573 16.5408 10.8867 16.5408C14.1007 16.5408 16.8447 16.0588 16.8447 14.1128C16.8447 12.1668 14.1177 11.6638 10.8867 11.6638Z"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    fillRule="evenodd"
                    clipRule="evenodd"
                    d="M10.8867 8.88788C12.9957 8.88788 14.7057 7.17888 14.7057 5.06888C14.7057 2.95988 12.9957 1.24988 10.8867 1.24988C8.77766 1.24988 7.06766 2.95988 7.06766 5.06888C7.05966 7.17088 8.75666 8.88088 10.8587 8.88788H10.8867Z"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M4.88472 7.89673C3.48872 7.70073 2.41572 6.50473 2.41272 5.05573C2.41272 3.62773 3.45372 2.44373 4.81872 2.21973"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                  <path
                    d="M3.04388 11.2502C1.69288 11.4522 0.749878 11.9252 0.749878 12.9002C0.749878 13.5712 1.19388 14.0072 1.91188 14.2812"
                    stroke="white"
                    strokeWidth="1.5"
                    strokeLinecap="round"
                    strokeLinejoin="round"
                  ></path>
                </svg>
              </span>
              <div>
                <h3
                  className="font-heading mb-4 text-2xl font-semibold"
                  data-config-id="header6"
                >
                  List of regular users
                </h3>
                <p className="text-lg text-gray-500" data-config-id="desc6">
                  Pellentesque eu quam vitae mi lacinia consequat quis in metus.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default MobileApp;
